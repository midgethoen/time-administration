...
function describe(e){
  return `${e.start} - ${e.stop} (${(e.duration/3600).toPrecision(2)}h)[${(e.tags||[]).join(',')}]`
}
...
const isNonBillable = e => (e.tags && (e.tags.includes(tags.traveling) || e.tags.includes(tags.break)))

const summarize = (filterFunc, entries) => R.pipe(
  R.filter(filterFunc),
  R.pluck('duration'),
  R.sum
)(entries)
...
async function main() {
  ...
  // get projects
  let projects = await request.get(`${apiUrl}/workspaces/${workspaceId}/projects`, options)
  const isClientId = R.contains(R.__, clients.map(c => c.id))
  const projectFilter = p => isClientId(p.cid)
  projects = projects.filter(projectFilter)

  // get entries for the given month
  const date = new Date
  const start_date =  new Date(date.getFullYear(), date.getMonth() - opts.month, 1);
  const end_date =    new Date(date.getFullYear(), date.getMonth() - opts.month + 1 , 1);
  const urlParams = qs.stringify({
    start_date: start_date.toISOString(),
    end_date: end_date.toISOString(),
  })
  let entries = await request.get(`${apiUrl}/time_entries?${urlParams}`, options)
  const isProjectId = R.contains(R.__, projects.map(c => c.id))
  ...
  results = R.pipe(
    ...
    R.map(([k, entries]) => {
      ...
      const insert = (e, patch) => ({
        ...
        desc: describe(e),
        time_entry: {
          ...R.pick(["wid", "pid", "uid", "billable", "description", "tags" ], e),
      ...
      entries.forEach(e=>{
        ...
        if (e.tags && e.tags.includes(tags.break)){
          if (breakLength >= maxbreakLength && e.billable){
            ...
            modifications.push(modify(e, { billable: false }))
          } else if (breakLength < maxbreakLength && (breakLength + e.duration) > maxbreakLength) {
            // is partially unbillable -> split
            const billableDuration = maxbreakLength - breakLength
            modifications.push(modify(e, {duration: billableDuration, billable: true}))
            modifications.push(insert(e, {
              start: addToDate(e.start, billableDuration).toISOString(),
              duration: e.duration - billableDuration,
              billable: false,
            }))
          } else if ((breakLength + e.duration) <= maxbreakLength && !e.billable) {
            // billable
            modifications.push(modify(e, { billable: true }))
          }
  ...
  if (opts.dry) {
    ...
    Promise.map(
      ...
      (mod) => {
        if (mod.type === 'insert'){
          return request.post(`${apiUrl}/time_entries`, {...options, body: { time_entry: mod.time_entry }});
        } else if (mod.type === 'modify') {
          return request.put(`${apiUrl}/time_entries/${mod.id}`, {...options, body: { time_entry: mod.patch }});
        }
      },
      {concurrency: 5}
    )
...
async function authenticate() {
  ...
  const response = await request.get(`${apiUrl}/me`, {...options, resolveWithFullResponse: true})
